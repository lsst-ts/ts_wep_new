...
class TestEstimateZernikesBase(lsst.utils.tests.TestCase):
    ...
    def testEstimateCornerZernikes(self):
        ...
        # Make sure the total rms error is less than 0.25 microns off
        # from the OPD truth as a sanity check
        self.assertLess(
            np.sqrt(np.sum(np.square(zernCoeffAvgR04 - trueZernCoeffR04))), 0.25
        )
        ...
        # from the OPD truth as a sanity check
        self.assertLess(
            np.sqrt(np.sum(np.square(zernCoeffAvgR40 - trueZernCoeffR40))), 0.25
        )
